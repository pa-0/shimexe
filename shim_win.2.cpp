...
tuple<unique_handle, unique_handle> MakeProcess(
  ...
  if (!CreatePipe(&m_hChildStd_OUT_Rd, &m_hChildStd_OUT_Wr, &saAttr, 0)) {
    cout <<
      "Could not create a pipe for the child process's STDOUT";
  ...
  if (!SetHandleInformation(m_hChildStd_OUT_Rd, HANDLE_FLAG_INHERIT, 0)) {
    cout <<
      "Could not ensure the handle to the pipe for STDOUT is not inherited.";
    return {move(processHandle), move(threadHandle)};
    // log error
    // return HRESULT_FROM_WIN32(GetLastError());
  }
  ...
  startInfo.hStdError = m_hChildStd_OUT_Wr;
  startInfo.hStdOutput = m_hChildStd_OUT_Wr;
